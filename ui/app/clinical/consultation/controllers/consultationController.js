...
angular.module('bahmni.clinical').controller('ConsultationController',
    ['$scope', '$rootScope', '$state', '$location', '$translate', 'clinicalAppConfigService', 'diagnosisService', 'urlHelper', 'contextChangeHandler',
        ...
        'ngDialog', '$filter', 'configurations', 'visitConfig', 'conditionsService', 'configurationService', 'auditLogService', 'patientService','observationsService','$timeout', '$http',
        function ($scope, $rootScope, $state, $location, $translate, clinicalAppConfigService, diagnosisService, urlHelper, contextChangeHandler,
            ...
            var apiUrl = Bahmni.Common.Constants.openmrsUrl+'/ws/rest/v1/';

            var isCAGPatient = function () {
                var data;
                var deferred = $q.defer();
                $http.get(apiUrl + 'cagPatient/' + $scope.patient.uuid)
                .then(function (response) {
                    data = response.data;
                    console.log(data);

                    if (data) {
                        console.log('Is CAG Patient');
                        deferred.resolve(true);
                    }
                    else {
                        console.log('Is not CAG patient');
            ...
            var saveCAG = function (toStateConfig, patientUuids) {
                ...
                // Iterate through patientUuids and process each patient
                return $q.all(patientUuids.map(function (patientUuid) {
                    return spinner.forPromise($q.all([preSavePromise(), encounterService.getEncounterType($state.params.programUuid, sessionService.getLoginLocationUuid())]).then(function (results) {
                        ...
                        // Update patient UUID in the encounter data
                        ...
                        return encounterService.create(encounterData)
                            .then(function (saveResponse) {
                                ...
                                auditLogService.log(patientUuid, "EDIT_ENCOUNTER", messageParams, "MODULE_LABEL_CLINICAL_KEY");
                                ...
                                return spinner.forPromise(diagnosisService.populateDiagnosisInformation(patientUuid, savedConsultation)
            ...
            $scope.save = function (toStateConfig) {
                appService.setOrderstatus(true);
                if (!isFormValid()) {
                    console.log('Link ' + Bahmni.Common.Constants.openmrsUrl);
                    $scope.$parent.$parent.$broadcast("event:errorsOnForm");
                    return $q.when({});
                }

                $q.all([isCAGPatient(),isCAGVisitOpen()])
                .then(function(results){
                    //check if isCAGPatient = result[0] and isCAGVisitOpen = result[1] are both true
                    if (results[0] && results[1]){
                        $http.get(apiUrl + 'cagVisit')
                        .then(function (response) {
                            ...
                            results.forEach(function(element) {
                                lookup[element.attender.uuid] = element;
                            });

                            var currentCagVisit = lookup[$scope.patient.uuid];

                            console.log('currentCAG', currentCagVisit);

                            var patientUuids = [];

                            for (let i=0; i<currentCagVisit.visits.length; i++){
                                patientUuids[i] = currentCagVisit.visits[i].patient.uuid;
                            }

                            console.log(patientUuids);
                            var currentPatientUuid = $scope.patient.uuid;
                            return saveCAG(toStateConfig, patientUuids);
                        });
                    }
